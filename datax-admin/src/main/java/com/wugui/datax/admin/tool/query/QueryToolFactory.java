...
public class QueryToolFactory {
    ...
    public static BaseQueryTool getByDbType(JobDatasource jobDatasource) {
        ...
        } else if (JdbcConstants.STARROCKS.equals(datasource)) {
            return getStarRocksQueryToolInstance(jobDatasource);
        }
    ...
    private static BaseQueryTool getStarRocksQueryToolInstance(JobDatasource jdbcDatasource) {
        try {
            return new StarRocksQueryTool(jdbcDatasource);
        } catch (Exception e) {
            throw RdbmsException.asConnException(JdbcConstants.STARROCKS,
                    e,jdbcDatasource.getJdbcUsername(),jdbcDatasource.getDatasourceName());
        }
    }
}
