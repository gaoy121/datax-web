...
public class DataxJsonHelper implements DataxJsonInterface {
    ...
    private StarRocksReaderDto starRocksReaderDto;
    private StarRocksWriterDto starRocksWriterDto;
    ...
    public void initReader(DataXJsonBuildDto dataxJsonDto, JobDatasource readerDatasource) {
        ...
        this.starRocksReaderDto = dataxJsonDto.getStarRocksReader();
        // reader 插件
        ...
        } else if (STARROCKS.equals(datasource)) {
            readerPlugin = new StarRocksReader();
            buildReader = buildReader();
        }
    }

    public void initWriter(DataXJsonBuildDto dataxJsonDto, JobDatasource jobDatasource) {
        this.writerDatasource = jobDatasource;
        //从数据源url中获取数据库名称
        String dataBaseName = jobDatasource.getJdbcUrl().substring(jobDatasource.getJdbcUrl().lastIndexOf("/") + 1);
        this.writerDatasource.setDatabaseName(dataBaseName);
        ...
        this.starRocksWriterDto = dataxJsonDto.getStarRocksWriter();
        // writer
        ...
        if (MYSQL.equals(datasource)) {
            ...
            writerPlugin = new StarRocksWriter();
            buildWriter = this.buildStarRocksWriter();
        }
    }
    ...
    @Override
    public Map<String, Object> buildStarRocksReader() {
        DataxStarRocksPojo dataxPluginPojo = new DataxStarRocksPojo();
        dataxPluginPojo.setJobDatasource(readerDatasource);
        dataxPluginPojo.setTables(readerTables);
        dataxPluginPojo.setRdbmsColumns(readerColumns);
        dataxPluginPojo.setSplitPk(starRocksReaderDto.getReaderSplitPk());
        if (StringUtils.isNotBlank(starRocksReaderDto.getQuerySql())) {
            dataxPluginPojo.setQuerySql(starRocksReaderDto.getQuerySql());
        }
        //where
        if (StringUtils.isNotBlank(starRocksReaderDto.getWhereParams())) {
            dataxPluginPojo.setWhereParam(starRocksReaderDto.getWhereParams());
        }
        return readerPlugin.buildStarRocks(dataxPluginPojo);
    }

    @Override
    public Map<String, Object> buildStarRocksWriter() {
        DataxStarRocksPojo dataxPluginPojo = new DataxStarRocksPojo();
        dataxPluginPojo.setJobDatasource(writerDatasource);
        dataxPluginPojo.setSelectedDatabase(writerDatasource.getDatabaseName());
        dataxPluginPojo.setTables(writerTables);
        dataxPluginPojo.setRdbmsColumns(writerColumns);
        dataxPluginPojo.setPreSql(starRocksWriterDto.getPreSql());
        dataxPluginPojo.setPostSql(starRocksWriterDto.getPostSql());
        dataxPluginPojo.setLoadUrl(starRocksWriterDto.getLoadUrl());
        return writerPlugin.buildStarRocks(dataxPluginPojo);
    }
